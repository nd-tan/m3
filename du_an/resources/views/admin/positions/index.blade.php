                            ...
                            <form action="{{ route('position.delete', $item->id) }}" method="post">
                                ...
                                @if (Auth::user()->hasPermission('Position_view'))
                                    <a style='color:rgb(52,136,245)' class='btn'
                                        href="{{ route('position.detail', $item->id) }}"><i class='bi bi-eye h4'></i></a>
                                @endif
                                @if (Auth::user()->hasPermission('Position_update'))
                                    <a style='color:rgb(52,136,245)' class='btn'
                                        href="{{ route('position.edit', $item->id) }}"><i
                                            class='bi bi-pencil-square h4'></i></a>
                                @endif
                                @if ($item->name != 'Supper Admin')
                                    @if (Auth::user()->hasPermission('Position_delete'))
                                        <button
                                            onclick="return confirm('Bạn có chắc muốn đưa chức vụ này vào thung rác không?');"
