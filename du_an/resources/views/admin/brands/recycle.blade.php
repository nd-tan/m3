...
<main id="main" class="main">
    ...
    <a class='btn btn' style='color:rgb(52,136,245)' href="{{route('brand.index')}}">Danh sách thương hiệu</a>
    ...
    <table class="table table-bordered border-primary" style=" text-align: center; width:100%">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td>{{$item->address}}</td>
            <td>
                <form action="{{route('brand.deleted',$item->id)}}" method="post">
                    @method('DELETE')
                    @csrf
                    {{-- <a style='color:rgb(52,136,245)' class='btn' href="#"><i class='bi bi-eye h4'></i></a> --}}
                    @if(Auth::user()->hasPermission('Brand_restore'))
                    <a onclick="return confirm('Bạn có chắc muốn khôi phục nhà cung cấp này không?');" style='color:rgb(52,136,245)' class='btn' href="{{route('brand.restore',$item->id)}}"><i class='bi bi-arrow-clockwise h4'></i></a>
                    @endif
                    @if(Auth::user()->hasPermission('Brand_forceDelete'))
