          ...
          <tr>
            ...
            <td>
                <form action="{{route('position.deleted',$item->id)}}" method="post" >
                    ...
                    <a onclick="return confirm('Bạn có chắc muốn khôi phục chức vụ này không?');" style='color:rgb(52,136,245)' class='btn' href="{{route('position.restore',$item->id)}}"><i class='bi bi-arrow-clockwise h4'></i></a>
                    @endif
                    @if(Auth::user()->hasPermission('Position_forceDelete'))
                    <button onclick="return confirm('Bạn có chắc muốn xóa chức vụ này không?');" class ='btn' style='color:rgb(52,136,245)' type="submit" ><i class='bi bi-trash h4'></i></button>
                    @endif
                </form>
            </td>
        </tr>
